...
def allElemsIntersectedOfCat(elem,catName,doc):
	...
	for _Cat in doc.Settings.Categories:
		if _Cat.Name == catName.ToString():
			rvt_Cat = _Cat	
	for intersect_elem in FilteredElementCollector(doc).OfCategoryId(_Cat.Id).WherePasses(ElementIntersectsElementFilter(elem)):#.ToElements()#.WhereElementIsNotElementType()	
		try:
